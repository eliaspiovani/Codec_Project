...
bool WriteOutput(int count, int charHSize)
{
    ...
    int currentBit = 0;
    char tempChar = NULL;
    //cout << EncodedHTree01 << endl << EncodedHTree02 << endl;
    
    for (int i = 0; i < EncodedHTree01.length(); i++) {
        if (EncodedHTree01[i] == '0') {
            buffer |= (false << (7 - currentBit));
            currentBit++;
        }
        else if (EncodedHTree01[i] == '1')
        {
            char C;
            ...
            C = EncodedHTree01[i];
            tempChar = EncodedHTree01[i] >> currentBit;
            cout << EncodedHTree01[i];
            buffer |= tempChar;
            output << buffer;
            buffer = NULL;
            C = EncodedHTree01[i];
            tempChar = EncodedHTree01[i] << (7 - currentBit +1);
            buffer |= tempChar;
    ...
    currentBit = 0;
    tempChar = NULL;
    for (int i = 0; i < EncodedHTree02.length(); i++) {
        if (EncodedHTree02[i] == '0') {
            buffer |= (false << (7 - currentBit));
            currentBit++;
        }
        else if (EncodedHTree02[i] == '1')
        {
            char C;
            buffer |= (true << (7 - currentBit));
            currentBit++;
            ...
            C = EncodedHTree02[i];
            tempChar = EncodedHTree02[i] >> currentBit;
            cout << EncodedHTree02[i];
            buffer |= tempChar;
            output << buffer;
            buffer = NULL;
            C = EncodedHTree02[i];
            tempChar = EncodedHTree02[i] << (7 - currentBit +1);
            buffer |= tempChar;
            //currentBit = 7 - currentBit;
    ...
    //cout << "positionsChar" << endl;
    //write all values of position
    buffer = NULL;
    currentBit = 0;
    
    for (std::list<string>::iterator it=positionsChar.begin(); it!=positionsChar.end(); ++it)
    {
        string temp = *it;
        string temp1, temp2;
        for (int i = 0; i < 10; i++) {
    ...
    if (currentBit != 0) output.write(&buffer, 1);
    //cout << endl << "lengthsChar" << endl;
    // all the values of length
    for (std::list<string>::iterator it=lengthsChar.begin(); it!=lengthsChar.end(); ++it)
    {
        string temp = *it;
        string temp1, temp2;
        for (int i = 0; i < 10; i++) {
    ...
    if (currentBit != 0) output.write(&buffer, 1);
    //cout << endl << "charsChar" << endl;
    // and all values for the nextChar
    for (std::list<char>::iterator it=charsChar.begin(); it!=charsChar.end(); ++it)
    {
        char temp = *it;
        string temp1;
